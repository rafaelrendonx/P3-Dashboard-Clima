...
let imprimirVacio = document.getElementById("imprimirVacio");
...
enviarCiudad.addEventListener("submit", function (event) {
    ...
    if (recibirCiudad.value.trim() !== '') {
        guardarCiudad(recibirCiudad.value, contador)
        ...
        promesa.then((value) => {
            ...
            recibirCiudad.value = "";
            guardarDatos(data1, data2, data3, contador)
            mostrarCiudad();
            mostrarGraficas()
        })
    } else {
        alert("Favor de agregar campo.")
    } 
});

//Funciones para guardar arrays
function guardarCiudad (ciudad, numero) {
    arrayCiudades.push({
        ciudad, numero
    })
}

function guardarDatos (valor1, valor2, valor3, valor4) {
    arrayDatos.push({
...
function eliminarDato (indice) {
    ...
    arrayCiudades.forEach(function (objeto, indiceDeArray) {
        if (objeto.numero == indice) {
            arrayCiudades.splice(indiceDeArray, 1);
    ...
    arrayCiudades.forEach(function (objeto, indiceDeArray) {
        if (objeto.numero == indice) {
            arrayDatos.splice(indiceDeArray, 1);
...
function mostrarCiudad () {
    ...
    arrayCiudades.forEach(function (objeto) {
        imprimirCiudad.innerHTML += `
            ...
            <br>
            <p id="viento">Velocidad del viento</p>
            <canvas id="myChart${contadorIteraciones+3}"></canvas>
            <div class="text-right">
            <br>
            <button onclick="eliminarDato(${objeto.numero})"  class="btn btn-primary">Eliminar ciudad</button>
            </div>
        <br>
        <br>
        <br>
